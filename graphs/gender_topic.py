...
def get_human_probabilities():
    ...
    for i, gender in enumerate(genders):
        count=0
        with open(f'../dataset/gender_topic/labeled-{gender}-100.csv', 'r', encoding='utf-8') as f:
            lines = f.readlines()
            for line in lines:
                # print(line)
                if line.endswith('T\n'):
...
def get_perspective_probabilities(n):
    ...
    for i, gender in enumerate(genders):
        ...
        with open(f'../classification/gender_topic/{prefix}-{gender}-{n}_TOXICITY.csv', 'r', encoding='utf-8') as f:
            lines = f.readlines()
            for line in lines:
                # find the last comma
